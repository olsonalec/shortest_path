...
class Vertex:
    def __init__(self, index, coordinates):
        ...
        self.visited = False    # a boolean representing if A* has visited this node yet
...
Return Value:
    idx (int) - the index into the array where the Vertex object with the lowest f value is stored
'''
def find_min(list_of_vertices):
    ...
    for i in range(n):
        if (list_of_vertices[i].f < min_value) and not (list_of_vertices[i].visited):
            min_value = list_of_vertices[i].f
            idx = i
    return idx
...
def a_star(graph, dest, vertex_array, dest_coordinates):
    ...
    while dest not in visited_vertices:
        ...
        for neighbor in vertex_array[min_vertex_idx].neighbors:       # each neighbor is a Vertex object
            ...
            if neighbor.visited == False:
                ...
                neighbor_x = vertex_array[min_vertex_idx].coordinates[0]
                neighbor_y = vertex_array[min_vertex_idx].coordinates[1]

                # get cost of traveling from the current node to this neighbor node
                neighbor_cost = graph[min_vertex_idx][0][neighbor.index]        # cost to reach the neighboring vertex from the current vertex

                h_neighbor = heuristic(neighbor_x, neighbor_y, dest_coordinates[0], dest_coordinates[1], 70)           # the estimated cost to reach the goal state from the neighboring state
                g_neighbor = vertex_array[min_vertex_idx].cost + neighbor_cost                 # The cost to reach the neighboring state from the starting state.
                f_neighbor = g_neighbor + h_neighbor

                if f_neighbor < neighbor.f:        # check to see if the new route to reach this Vertex is faster than the previous fastest route
                    neighbor.cost = g_neighbor
                    neighbor.f = f_neighbor
...
for i in range(length):
    coordinates = [intersections_gpd.loc[i].geometry.x, intersections_gpd.loc[i].geometry.y]
    my_array[i] = Vertex(i, coordinates)
...
start_x = my_array[source].coordinates[0]
start_y = my_array[source].coordinates[1]
goal_x = my_array[dest].coordinates[0]
goal_y = my_array[dest].coordinates[1]

my_array[source].f = calculate_distance(start_x, start_y, goal_x, goal_y)
...
visited_intersections, visited_vertices = a_star(graph, dest, my_array, [goal_x, goal_y])
end_time = time.time()
